...
public class NettyNode extends AsyncService {
    ...
    static {
        try {
            APPEND_ENTRY = RaftService.class.getDeclaredMethod("appendEntries",
                    long.class, ID.class, long.class, long.class, Entry[].class, long.class);  // 复制日志+发送心跳（由 leader 调用）
            PREPARE_VOTE = RaftService.class.getDeclaredMethod("preVote",
                    long.class, ID.class, long.class, long.class);  // 预选（由 follower 调用）
            REQUEST_VOTE = RaftService.class.getDeclaredMethod("requestVote",
                    long.class, ID.class, long.class, long.class);  // 选主（由 candidate 调用）
            INSTALL_SNAP = RaftService.class.getDeclaredMethod("installSnapshot",
                    long.class, ID.class, long.class, long.class);  // 同步快照（由 leader 调用）
    ...
    public Callable prepareLocalInvocation(String scope, Object data, RaftContext context) {
        ...
        if (scope.startsWith(PREPARE_VOTE.getName())) {
            Object[] params = (Object[]) data;
            if (params.length != 4) {
                throw new IllegalArgumentException("非法参数大小 " + params.length);
            }
            long term = ((Number) params[0]).longValue();
        ...
        if (scope.startsWith(REQUEST_VOTE.getName())) {
            ...
            long lastLogIndex = ((Number) params[2]).longValue();
            long lastLogTerm = ((Number) params[3]).longValue();
            return () ->  context.participant().requestVote(term, candidateId, lastLogIndex, lastLogTerm); //参与者发起选票
        }
        if (scope.startsWith(INSTALL_SNAP.getName())) {
            Object[] params = (Object[]) data;
            if (params.length != 4) {
        ...
        throw new IllegalArgumentException("未知内容 无法解析 域" + scope);
    }
